...
export default function App() {
  ...
  const [error, setError] = useState(''), [canRetry, setCanRetry] = useState(false), [editing, setEditing] = useState<string>(), controller = useRef<AbortController | undefined>(undefined)
  ...
  const reload = async () => { const s = await store.snapshot(); setCharacters(s.characters); setPersonas(s.personas); setPresets(s.presets); setSessions(s.sessions); setMessages(s.messages); setSettings(s.settings) }
  ...
  async function generate(mode: 'send' | 'regenerate' | 'retry' = 'send') {
    ...
    try { await streamCompletion(settings, apiKey, composePrompt(character, personas.find(x => x.isDefault), history, settings), controller.current.signal, text => { reply.content += text; reply.updatedAt = Date.now(); setMessages(old => old.map(x => x.id === reply.id ? { ...reply } : x)) }); await store.saveMessage(reply) }
  ...
  async function saveSettings(next = settings) { await store.saveSettings(next); await store.setApiKey(apiKey.trim()); setError('设置已保存'); setTimeout(() => setError(''), 1500) }
  ...
  return <div className="app">
    ...
    {error && <div className={error === '设置已保存' ? 'notice success' : 'notice'}><span>{error}</span>{canRetry && <button onClick={() => generate('retry')}><RefreshCw/>重试</button>}<button aria-label="关闭" onClick={() => setError('')}><X size={16}/></button></div>}
    <main className="content">
      ...
      {tab === 'settings' && <SettingsView settings={settings} setSettings={setSettings} apiKey={apiKey} setApiKey={setApiKey} personas={personas} presets={presets} reload={reload} save={() => saveSettings()}/>} 
    </main>
...
function SettingsView({ settings, setSettings, apiKey, setApiKey, personas, presets, reload, save }: { settings: AppSettings; setSettings: (x: AppSettings) => void; apiKey: string; setApiKey: (x: string) => void; personas: Persona[]; presets: Preset[]; reload: () => Promise<void>; save: () => void }) {
