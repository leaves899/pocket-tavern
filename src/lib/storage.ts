...
async function snapshot(): Promise<Snapshot> {
  ...
  const [characters, personas, presets, worldBookEntries, sessions, messages, settingRows] = await Promise.all([
    payloads<Character>('characters', 'ORDER BY updated_at DESC'), payloads<Persona>('personas'), payloads<Preset>('presets'),
    payloads<WorldBookEntry>('world_book_entries', 'ORDER BY priority DESC,created_at ASC'),
...
async function migrateLegacySnapshot() {
  ...
  for (const x of legacy.worldBookEntries || []) await upsert('world_book_entries', x.id, x, 'priority,created_at', [x.priority, x.createdAt])
...
export const store = {
  async init() {
    if (!native) {
      ...
      web.presets ||= []; web.worldBookEntries ||= []; web.personas ||= emptySnapshot().personas; web.settings ||= { ...defaultSettings }; await webPersist(web); return
    }
    ...
    await db.execute(`
      ...
      CREATE TABLE IF NOT EXISTS world_book_entries(id TEXT PRIMARY KEY NOT NULL,payload TEXT NOT NULL,priority INTEGER NOT NULL,created_at INTEGER NOT NULL);
      ...
      CREATE INDEX IF NOT EXISTS idx_world_book_priority ON world_book_entries(priority DESC,created_at ASC);
